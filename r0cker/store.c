...
void store_write(store_t *store, char *key, size_t key_len, char *buf, size_t buf_len) {
    ...
    LOG_DEBUGF("store.c", "uuid: %s", uuid_str);
    LOG_DEBUGF("store.c", "index path %s", ScanCtx.index.path);
    
    char * thumbs_dir = malloc(1000);
    strcpy(thumbs_dir, ScanCtx.index.path);
    strncat(thumbs_dir, "thumbnails/",1000);
    LOG_DEBUGF("store.c", "thumbs dir %s", thumbs_dir);


    struct stat st={0};

    if (stat(thumbs_dir, &st) == -1) {
      mkdir(thumbs_dir,0700);
    }

    strncat(thumbs_dir, uuid_str,1000);
    strncat(thumbs_dir, ".jpeg", 1000);
    FILE *write_ptr;

    write_ptr=fopen(thumbs_dir,"wb");
