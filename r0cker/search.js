...
let mimeTree;
let tagTree;
const server_path= "https://node1.searchevolution.com:9200/sist2/";
...
function saveTag(tag, hit) {
    ...
    return $.jsonPost(server_path + "_update/" + hit["_id"], {
       script 
    }).then(() => {
        tagBar.blur();
        $("#tagModal").modal("hide");
...
function search(after = null) {
    ...
    $.jsonPost(server_path + "_search", q).then(searchResult => {
